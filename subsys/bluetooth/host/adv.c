				      ...
				      const struct bt_ad *ad, size_t ad_len)
{
	...
	BT_DBG("ad_len: %d", ad_len);
	for (size_t i = 0; i < ad_len; i++) {

		const struct bt_data *data = ad[i].data;

		BT_DBG("ad[%d].len: %d", i, ad[i].len);
		for (size_t j = 0; j < ad[i].len; j++) {
			size_t len = data[j].data_len;
			uint8_t type = data[j].type;
			size_t offset = 0;
			...
			while (offset < data[j].data_len) {
				...
				BT_WARN("Fragmented adv: op: %d, len: %d", set_data->op, set_data->len);
				BT_HEXDUMP_DBG(buf->data, buf->len, "buf");
				__ASSERT(set_data->len <= BT_HCI_LE_EXT_ADV_FRAG_MAX_LEN,
					 "Expected length to be <=251, got %d", set_data->len);
